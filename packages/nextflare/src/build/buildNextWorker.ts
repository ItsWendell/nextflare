...
export const buildNextWorker = async (options?: VercelBuildPagesOptions) => {
    ...
    const middlewareManifest = require(path.join(process.cwd(), ".next", "server", "middleware-manifest.json"));
    ...
    const requires = Object.entries(middlewareManifest.functions).reduce((acc, [name, func]: [string, any]) => {
    ...
    const result = await build({
      ...
      stdin: {
        contents: `
          ${Array.from(requires).map((file) => `import "./${file}";`).join("\n")}
          ${workersFile}
        `,
        loader: "ts",
